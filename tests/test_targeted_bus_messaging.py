...
def test_finds_sender_by_change_field(tmp_path: Path) -> None:
    ...
    _write_msg(
        bus,
        "20260101T120000-maestro-to-cli-agent-task-assignment",
        {
...
def test_falls_back_to_human_when_no_task_assignment(tmp_path: Path) -> None:
    ...
    _write_msg(
        bus,
        "20260101T120000-specs-spec-change",
        {
            "type": "spec-change",
            "from": "otaman-specs",
            "to": "all",
...
def _setup_project(tmp_path: Path, agent: str) -> Path:
    ...
    (meta / "platform.yaml").write_text("project: test\nrepos: []\n")
