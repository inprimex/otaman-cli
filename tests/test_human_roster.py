...
from otaman_cli.doctor import check_human_roster
from otaman_cli.onboard.program_init.runner import _VALID_ROSTER_ROLES, _prompt_human_roster
from otaman_cli.onboard.program_init.platform_gen import write_platform_yaml
from otaman_cli.pm.cmd_init import _resolve_roster_pm_user_ids
...
class TestRosterResolver:
    ...
    def test_dry_run_does_not_call_adapter(self, tmp_path: Path):
        platform = self._write_platform(tmp_path, [
    ...
    def test_resolves_by_email_and_writes_back(self, tmp_path: Path, monkeypatch):
        ...
        class _StubEntry:
            def __init__(self, name, email, roles, pm_user_id=None):
                self.name = name; self.email = email; self.roles = roles; self.pm_user_id = pm_user_id
        ...
        _resolve_roster_pm_user_ids(
            platform, adapter=MagicMock(), dry_run=False, UI=mock_UI,
        )
        ...
        config = {
            ...
            "human-roster": [
                {"name": "Alice", "email": "a@x.com",
                 "roles": ["cto", "sales-lead"], "pm-user-id": 7},
            ],
        }
        result = check_human_roster(config)
