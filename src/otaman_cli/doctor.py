...
def check_repo_owner_registration(
    repos: list[dict[str, Any]], known_agents: set[str]
) -> dict[str, Any]:
    """ERROR on a ``repos[].owner`` that is not registered in agents.yaml
    (identity-divergence 1.1).

    The pmeets incident chain started here: nothing validated a repo's ``owner:``
    against the agent registry, so a divergent laptop platform.yaml introduced a
    name that then materialized as a phantom agent in status surfaces for 17
    hours. Canon: an unknown owner is an ERROR naming the repo and the owner —
    never an implicit new agent.

    No agents.yaml (or an empty one) means there is no registry to check against,
    so the check is a no-op — same convention as
    :func:`check_launch_command_agent_names`.
    """
    result: dict[str, Any] = {"check": "repo_owner_registration", "status": "ok", "details": {}}
    if not known_agents:
        return result
    issues = []
    unknown: list[str] = []
    for repo in repos:
        ...
        if not owner or owner in known_agents:
            continue
        unknown.append(f"{name} -> {owner}")
        issues.append(
            {
                "issue": (
                    f"repo `{name}`: owner {owner!r} is not registered in "
                    ".agents/agents.yaml (an unregistered owner must never become "
                    "an implicit agent — this is the phantom-agent source)"
                ),
                "fix": (
                    f"Add {owner!r} to .agents/agents.yaml, or correct repo "
                    f"'{name}' to an owner listed there "
    ...
    if issues:
        result["status"] = "fail"
        result["issues"] = issues
        result["details"]["unknown_owners"] = unknown
    return result


def check_tmux_global_agent_env() -> dict[str, Any]:
    """WARN when ``OTAMAN_AGENT`` exists in the tmux SERVER-GLOBAL environment
    (identity-divergence 1.4).

    Identity must be process-scoped: per pane / per process. A server-global
    value is shared mutable state with last-writer-wins semantics — every pane
    spawned afterwards inherits whichever agent wrote it last, which is how one
    agent's identity leaked into another's session. Tolerant of a missing tmux or
    no running server (then there is nothing to warn about).
    """
    result: dict[str, Any] = {"check": "tmux_global_agent_env", "status": "ok", "details": {}}
    if not _which("tmux"):
        return result
    rc, out, _ = _run(["tmux", "show-environment", "-g", "OTAMAN_AGENT"])
    if rc != 0:
        # no server running, or the variable is unset/removed ("-OTAMAN_AGENT")
        return result
    line = (out or "").strip()
    if not line.startswith("OTAMAN_AGENT="):
        return result
    ...
    result["issues"] = [
        {
            "issue": (
                f"tmux server-global OTAMAN_AGENT={value!r} is set — identity must be "
                "process-scoped (per pane/process), never server-global: it is shared "
                "mutable state and the last writer wins for every pane spawned after"
            ),
            "fix": "tmux set-environment -g -u OTAMAN_AGENT",
            "severity": "medium",
        }
    ]
    return result
...
def _status_dir(project_root: Path) -> Path:
    return project_root / ".agents" / "status"


#: Status writers that are legitimately absent from agents.yaml. ``human`` is a
#: first-class identity across the platform — a bus recipient (`otaman send
#: human`), the `afk` state writer, the identity fallback in propose/blocked, and
#: a routing-rule target generated by `otaman init` — but it is deliberately NOT
#: an agent registry entry. Reaping its status file would delete live AFK state,
#: so it is never an orphan.
RESERVED_STATUS_IDENTITIES = frozenset({"human"})


def find_orphan_status_files(project_root: Path, known_agents: set[str]) -> list[Path]:
    """``.agents/status/*.yaml`` files whose agent has no agents.yaml entry.

    The phantom agent persisted because nothing could SEE the orphan: it had a
    status file but no registry entry. Files are matched by stem (the backend
    writes one singleton ``<agent>.yaml`` per agent); in-flight atomic-write temp
    files (``.<agent>.*.yaml.tmp``) and :data:`RESERVED_STATUS_IDENTITIES` are
    skipped. An empty registry means nothing is knowable as an orphan, so the
    result is empty.
    """
    if not known_agents:
        return []
    status_dir = _status_dir(project_root)
    ...
    for path in sorted(status_dir.glob("*.yaml")):
        if path.name.startswith(".") or path.name.endswith(".tmp"):
            continue
        if path.stem in RESERVED_STATUS_IDENTITIES:
            continue
        if path.stem not in known_agents:
            orphans.append(path)
    return orphans


def check_orphan_status_files(project_root: Path, known_agents: set[str]) -> dict[str, Any]:
    """Report status files with no agents.yaml entry (identity-divergence 1.4).

    Reported, not auto-removed — ``otaman cleanup`` reaps them, naming each.
    """
    result: dict[str, Any] = {"check": "orphan_status_files", "status": "ok", "details": {}}
    orphans = find_orphan_status_files(project_root, known_agents)
    if not orphans:
        return result
    result["status"] = "warn"
    result["details"]["orphans"] = [p.name for p in orphans]
    result["issues"] = [
        {
            "issue": (
                f"status file `{p.name}` has no agents.yaml entry — a phantom agent "
                f"({p.stem!r}) is visible in status surfaces with nothing backing it"
            ),
            "fix": "otaman cleanup  (removes orphaned status files, naming each)",
            "severity": "medium",
        }
        for p in orphans
    ]
...
def run_doctor(project_root: Path) -> dict[str, Any]:
    ...
    known_agents = _load_known_agents_safe(project_root)
    ...
    checks = [
        ...
        check_launch_command_agent_names(repos, known_agents),
        check_repo_owner_registration(repos, known_agents),
