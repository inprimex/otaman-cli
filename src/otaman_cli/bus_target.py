"""Cross-program send resolution (single-bus-per-program tasks 2.1-2.3).

Consumes ``otaman_core.bus.uri`` (the pure addressing layer, task 1.1) and
adds the filesystem half the spec assigns to otaman-cli: local-context
derivation, target-program root resolution via the DECLARED org layout,
and fail-closed enforcement of the target's ``bus.boundaries``.

The P1 split-brain lesson governs everything here: resolution interprets
the declared layout (``orgs/<org>/programs/<program>/<meta-dir>``, org
config ``programs:`` overrides, the ``<program>-otaman`` convention) and
NEVER walks up or scans for ``.agents`` roots. A target that cannot be
resolved from declarations is refused with guidance, not discovered.

MCP parity note: ``otaman_send`` (otaman-plugin's bus_server.py) must
apply the same resolution. This module is import-clean of otaman-cli's UI
layer precisely so the plugin server can import or port it wholesale —
same pattern as cc_fanout.py, same manual-sync chore until the planned
otaman-core extraction.
"""
