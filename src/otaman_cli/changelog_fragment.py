"""Changelog-fragment discipline for shipped-code PRs (release-notes-fragments 1.2).
...
This module is the merge-time half — the predicate CI blocks on. It is pure: no
git, no I/O, no policy loading. The caller supplies the changed paths, the
resolved policy rules and any exemption text; everything here is a decision over
those inputs, which is what makes the shipped-code classification testable
rather than a regex buried in a workflow file.

The config shape (``dir``, ``filename``, ``categories``, ``exemption_marker``)
is core's, from ``GIT_STANDARD_RULES["changelog_fragment"]`` — read, never
redefined, so the scaffolder (plugin 1.4), this check and the release assembly
(deploy 1.3) cannot disagree about where fragments live.
"""
