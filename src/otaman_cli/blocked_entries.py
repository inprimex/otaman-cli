"""One parser for `.agents/blocked/<agent>.md` entries (blocked-entry-lifecycle).

Entries were parsed by ad-hoc regexes in at least five places — three in
``commands/blocked.py``, one in ``commands/complete.py``, one in
``commands/check.py`` — each with its own idea of what an entry is and how to
recognise a tombstone. That duplication is why the surface drifted: the
``complete`` clearer matched entries by looking for the CHANGE NAME inside the
title, while ``propose`` titles entries by proposal title, so it could never fire
(deploy-agent's 16-entry file, 5/5 sampled already archived).

This module is the single definition. It is pure text-in/text-out: no I/O, no
clock, no policy about WHEN an entry terminates — callers own that.

Entry shape (unchanged, additive only per the re-scope):

    ## Blocked: <human title>
    - **Proposal**: 20260913T144246-cli-agent-to-human-spec-change-request
    - **Kind**: awaiting-approval          <- added by 1.1; inferred when absent
    - **Blocked since**: 2026-09-13T14:42:46Z

A TOMBSTONED entry is the same block wrapped in an HTML comment with a
``cleared <date> — <reason>`` trailer — plugin's format, matched here rather than
redefined, so an entry cleared by either side reads the same to both.
"""
...
#: A live entry starts a line; a tombstoned one is preceded by the comment open.
...
class BlockedEntry:
    ...
    def kind(self) -> str:
        ...
        Legacy entries predate the field (1.1 is additive), so they are read, not
        rejected: one carrying a proposal ref is an approval wait — that is what
...
def parse_entries(text: str, *, include_tombstoned: bool = False) -> list[BlockedEntry]:
    ...
    every read-side caller wants live blocks. Migration (1.5) passes
    ...
    for match in _ENTRY_RE.finditer(text or ""):
        ...
        body = match.group(2)
        block = match.group(0)
        title = (body.splitlines() or [""])[0].strip()
...
def render_entry(
    ...
    *,
    ...
    """One entry block, with the ``Kind`` field 1.1 adds.
