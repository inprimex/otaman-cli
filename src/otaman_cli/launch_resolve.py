...
def emit_exports(state: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append(f"export MAESTRO_ACTIVE_CONNECTION={_bash_single_quote(state['connection_name'])}")
    lines.append(f"export MAESTRO_ACTIVE_ACCOUNT={_bash_single_quote(state['account_name'])}")
    lines.append(f"export MAESTRO_CONNECTION_TYPE={_bash_single_quote(state['connection_type'])}")
