...
def _ensure_routing_rules(platform_yaml: Path) -> int:
    ...
    # zero formatting impact on the rest of the file.
    if "bus" not in doc:
    ...
    # Path B — `bus:` exists, need to add missing rule(s) into it.  Round-trip
    # via ruamel.yaml.  Acceptable re-flow tradeoff (rare path).
    try:
        ...
        for r in missing:
            doc_rt["bus"]["routing_rules"].append(r)
        out = _io.StringIO()
        rt.dump(doc_rt, out)
        platform_yaml.write_text(out.getvalue(), encoding="utf-8")
        return 1
    except Exception:
        # Fallback: append plain-text under an inline second `bus:` block;
        # YAML parsers will reject duplicate keys, so prefer to skip silently
        # rather than corrupt the file. The user will need to merge by hand.
        return 0
