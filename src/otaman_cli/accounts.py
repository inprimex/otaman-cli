...
Entry points are invoked from the otaman CLI;  # legacy: was cli/maestro.py each subcommand accepts a
...
_MARKER_BEGIN = "# BEGIN MAESTRO ACCOUNTS — generated by `maestro accounts install-shell-aliases`"  # legacy: marker string must remain stable for idempotency
...
def install_shell_aliases(
    ...
    if not records:
        raise RuntimeError(
            ...
            f"Add one first with 'otaman accounts add'."
        )
...
def _build_telegram_block_yaml(
    ...
    lines.extend([
        ...
        f"{indent}    - {{ type: keyring, service: otaman, account: tg-{bot_token_env.lower()} }}",
    ])
    return "\n".join(lines) + "\n"
...
def configure_telegram(
    ...
    *,
    ...
    results.append(
        f"  1. Put the bot token in the otaman folder's .otaman/secrets.env:"
    )
    results.append(f"       {bot_token_env}=<your-bot-token-here>")
    results.append(f"       chmod 600 .otaman/secrets.env")
