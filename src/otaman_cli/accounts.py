"""otaman accounts — add / list / remove / install-shell-aliases.
...
Entry points are invoked from the otaman CLI;  # legacy: was cli/maestro.py
each subcommand accepts a
...
def resolve_settings_path(cli_override: str | None = None) -> Path:
    ...
    Priority:
      ...
      2. ``<otaman-root>/launch-settings.yaml``
...
def _find_account_span(
    lines: list[str], accounts_span: tuple[int, int], name: str
) -> tuple[int, int] | None:
...
def add_account(
    ...
    if isinstance(accounts, dict) and name in accounts:
        raise ValueError(
            ...
            f"Remove it first with 'otaman accounts remove {name}'."
        )
...
_MARKER_BEGIN = (
    "# BEGIN MAESTRO ACCOUNTS — generated by "  # legacy: marker must remain stable
    "`maestro accounts install-shell-aliases`"  # legacy: for idempotency
)
_MARKER_END = "# END MAESTRO ACCOUNTS"  # legacy: marker string must remain stable for idempotency
...
def render_aliases_block(records: list[dict[str, Any]], shell: str) -> str:
    ...
    if shell == "powershell":
        for r in records:
            ...
            lines.append("    $prev = $env:CLAUDE_CONFIG_DIR")
            lines.append(f"    $env:CLAUDE_CONFIG_DIR = '{config_dir}'")
            lines.append("    try { claude @args } finally { $env:CLAUDE_CONFIG_DIR = $prev }")
            lines.append("}")
            lines.append("")
    elif shell == "fish":
        for r in records:
...
def install_shell_aliases(
    ...
    if shell not in {"bash", "zsh", "fish", "powershell"}:
        raise ValueError(f"--shell must be one of: bash, zsh, fish, powershell (got {shell!r})")

    records = list_accounts(settings_path)
    if not records:
        raise RuntimeError(
            f"No accounts configured in {settings_path}. Add one first with 'otaman accounts add'."
        )
...
def _parse_user_ids(raw: str) -> list[int]:
    ...
    for item in (raw or "").split(","):
        ...
        try:
            ...
            raise ValueError(f"invalid user id {item!r}: must be an integer") from exc
    return ids
...
def _build_telegram_block_yaml(
    ...
    lines.extend(
        [
            ...
            f"{indent}    - {{ type: keyring, service: otaman, "
            f"account: tg-{bot_token_env.lower()} }}",
        ]
    )
    return "\n".join(lines) + "\n"
    ...
    *,
    ...
    if not settings_path.exists():
        raise FileNotFoundError(
            f"{settings_path} does not exist — run `otaman accounts add {account}` first"
        )
    ...
    if not isinstance(accounts, dict) or account not in accounts:
        raise KeyError(
            ...
            f"Run `otaman accounts add {account} --config-dir ~/.claude-{account}` first."
        )
    ...
    results.append("  1. Put the bot token in the otaman folder's .otaman/secrets.env:")
    results.append(f"       {bot_token_env}=<your-bot-token-here>")
    results.append("       chmod 600 .otaman/secrets.env")
    results.append(
        "  2. `pip install otaman-bridge[telegram]` "
        "(or `uv pip install otaman-bridge[telegram]` — faster)"
    )
    results.append(f"  3. `otaman bridge run --account {account}`")
    results.append("  4. `otaman afk on 30m`")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="otaman accounts",
        description="Manage Claude Code account definitions in launch-settings.yaml",
    )
    parser.add_argument(
        "--settings",
        help=f"Path to {DEFAULT_SETTINGS_FILENAME} (default: otaman-root/launch-settings.yaml)",
    )
